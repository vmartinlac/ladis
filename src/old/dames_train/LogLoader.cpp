...
bool LogLoader::load(const QString& path)
{
...

    {
        ...
        while(go_on)
        {
            ...
            if(dump)
            {
                const int index_from = myStatesFrom.size();
                ...
                while(it2 != current_states_to.end())
                {
                    ...
                    myStatesTo.back().index_from = index_from;
    ...
    */
    ...
    std::cout << "NumStatesFrom: " << myStatesFrom.size() << std::endl;
    std::cout << "NumStatesTo: " << myStatesTo.size() << std::endl;

    return true;
}

void LogLoader::getExample(size_t index, Checkers::State& state_from, Checkers::State& state_to, float& transition_probability) const
{
    state_to = myStatesTo[index].state;
    state_from = myStatesFrom[myStatesTo[index].index_from].state;
    transition_probability = myStatesTo[index].probability;
}

size_t LogLoader::getNumExamples() const
{
    return myStatesTo.size();
}

void LogLoader::parseRootDirectory(const QString& path, std::multimap<Checkers::State, Checkers::State>& seen)
{
...
void LogLoader::parseSubdirectory(const QString& path, std::multimap<Checkers::State, Checkers::State>& seen)
{
    QDir dir;
    if(!dir.cd(path)) throw std::runtime_error("could not access directory");
...
void LogLoader::parseFile(const QString& path, std::multimap<Checkers::State, Checkers::State>& seen)
{
